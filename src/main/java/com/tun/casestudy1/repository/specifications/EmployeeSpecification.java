            ...
            if (searchValue != null && !searchValue.isEmpty()) {
                if ("all".equals(filterType)) {
                    ...
                    switch (filterType) {
                        ...
                        case "dateOfBirth" ->
                                predicates.add(criteriaBuilder.like(criteriaBuilder.function("DATE_FORMAT", String.class,
                                        root.get("dOB"), criteriaBuilder.literal("%d/%m/%Y")), "%" + searchValue + "%"));
                        case "department" ->
                                predicates.add(criteriaBuilder.like(root.get("department").get("name"), "%" + searchValue + "%"));
                        case "level" ->
                                predicates.add(criteriaBuilder.like(root.get("level").as(String.class), "%" + searchValue + "%"));
                        case "phoneNumber" ->
                                predicates.add(criteriaBuilder.like(root.get("phoneNumber"), "%" + searchValue + "%"));
                        case "salary" ->
                                predicates.add(criteriaBuilder.like(root.get("salary").as(String.class), "%" + searchValue + "%"));
                        default -> {
                        }
                    }
                }
